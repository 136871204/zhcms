...
<?php }?>
            <table cellpadding="3" cellspacing="1">
                <tr>
                    <th><a href="javascript:listTable.sort('ad_name'); ">广告名称</a><?php echo $sort_ad_name;?></th>
                    <th><a href="javascript:listTable.sort('position_id'); ">广告位置</a><?php echo $sort_position_id;?></th>
                    <th><a href="javascript:listTable.sort('media_type'); ">媒介类型</a><?php echo $sort_media_type;?></th>
                    <th><a href="javascript:listTable.sort('start_date'); ">开始日期</a><?php echo $sort_start_date;?></th>
                    <th><a href="javascript:listTable.sort('end_date'); ">结束日期</a><?php echo $sort_end_date;?></th>
                    <th><a href="javascript:listTable.sort('click_count'); ">点击次数</a><?php echo $sort_click_count;?></th>
                    <th>生成订单</th>
                    <th>操作</th>
                </tr>
                <?php if(!empty($ads_list)){?>
                    <?php if(is_array($ads_list)):?><?php $index=0; ?><?php  foreach($ads_list as $list){ ?>
                    <tr>
                        <td class="first-cell">
                            <span onclick="javascript:listTable.edit(this, 'edit_ad_name', <?php echo $list['ad_id'];?>)"><?php echo $list['ad_name'];?></span>
                        </td>
                        <td align="left">
                            <span><?php if($list['position_id'] == 0){?>站外广告<?php  }else{ ?><?php echo $list['position_name'];?><?php }?></span>
                        </td>
                        <td align="left"><span><?php echo $list['type'];?></span></td>
                        <td align="center"><span><?php echo $list['start_date'];?></span></td>
                        <td align="center"><span><?php echo $list['end_date'];?></span></td>
                        <td align="right"><span><?php echo $list['click_count'];?></span></td>
                        <td align="right"><span><?php echo $list['ad_stats'];?></span></td>
                        <td align="right">
                            <span>
                                <?php if($list['position_id'] == 0){?>
                                    <a href="<?php echo U('index');?>&act=add_js&type=<?php echo $list['media_type'];?>&id=<?php echo $list['ad_id'];?>" title="生成并复制JS代码">
                                        <img src="http://www.works.com/zh/Zhcms/Admin/Tpl/Public/ec/images/icon_js.gif" border="0" height="16" width="16" />
                                    </a>
                                <?php }?>
                                <a href="<?php echo U('index');?>&act=edit&id=<?php echo $list['ad_id'];?>" title="编辑">
                                    <img src="http://www.works.com/zh/Zhcms/Admin/Tpl/Public/ec/images/icon_edit.gif" border="0" height="16" width="16" />
                                </a>
                                <a href="javascript:;" onclick="listTable.remove(<?php echo $list['ad_id'];?>, '您确实要删除该属性吗？')" title="<?php echo $lang['remove'];?>">
