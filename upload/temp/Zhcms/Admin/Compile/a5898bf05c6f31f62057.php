    ...
    <form action="<?php echo U('index');?>" method="post" name="theForm" onsubmit="return validate()">
        <table width="100%">
            <tr >
                ...
                <td>
                    <input type="text" name="reg_field_name" value="<?php echo $reg_field['reg_field_name'];?>" maxlength="20" />
                    <span class="require-field">*</span>
                </td>
            </tr>
            <tr >
                <td class="label">排序权值: </td>
                <td><input type="text" name="reg_field_order" value="<?php echo $reg_field['reg_field_order'];?>" maxlength="3" size="5"/></td>
            </tr>
            <tr >
                <td class="label">是否显示: </td>
                <td>
                    <input type="radio" name="reg_field_display" value="1" <?php if($reg_field['reg_field_display'] == 1){?>checked='checked'<?php }?>/>是&nbsp;&nbsp;&nbsp;
                    <input type="radio" name="reg_field_display" value="0" <?php if($reg_field['reg_field_display'] == 0){?>checked='checked'<?php }?>/>否
                </td>
            </tr>
            <tr >
                <td class="label">是否必填: </td>
                <td>
                    <input type="radio" name="reg_field_need" value="1" <?php if($reg_field['reg_field_need'] == 1){?>checked='checked'<?php }?>/>是&nbsp;&nbsp;&nbsp;
                    <input type="radio" name="reg_field_need" value="0" <?php if($reg_field['reg_field_need'] == 0){?>checked='checked'<?php }?>/>否
                </td>
            </tr>
            <tr>
                <td></td>    
                <td align="left">
                    ...
                    <input type="hidden" name="id" value="<?php echo $reg_field['reg_field_id'];?>" />
                    <input type="hidden" name="old_field_name" value="<?php echo $reg_field['reg_field_name'];?>" />
