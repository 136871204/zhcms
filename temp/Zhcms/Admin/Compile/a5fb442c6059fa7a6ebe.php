            ...
            <table cellpadding="3" cellspacing="1">
                <tr>
                    <th> 
                        ...
                        <a href="javascript:listTable.sort('suppliers_id'); ">编号</a><?php echo $sort_suppliers_id;?> 
                    </th>
                    <th><a href="javascript:listTable.sort('suppliers_name'); ">供货商名称</a><?php echo $sort_suppliers_name;?></th>
                    <th>供货商描述</th>
                    <th>状态</th>
                    <th>操作</th>
                </tr>
                <?php if(!empty($suppliers_list)){?>
                    <?php if(is_array($suppliers_list)):?><?php $index=0; ?><?php  foreach($suppliers_list as $suppliers){ ?>
                    <tr>
                        <td>
                            <input type="checkbox" name="checkboxes[]" value="<?php echo $suppliers['suppliers_id'];?>" />
                            <?php echo $suppliers['suppliers_id'];?>
                        </td>
                        <td class="first-cell">
                            <span onclick="javascript:listTable.edit(this, 'edit_suppliers_name', <?php echo $suppliers['suppliers_id'];?>)"><?php echo $suppliers['suppliers_name'];?></span>
                        </td>
                        <td><?php echo nl2br($suppliers['suppliers_desc']);?></td>
                        <td align="center">
                            <img src="http://www.metacms.com/zh/Zhcms/Admin/Tpl/Public/ec/images/<?php if($suppliers['is_check'] == 1){?>yes<?php  }else{ ?>no<?php }?>.gif" onclick="listTable.toggle(this, 'is_check', <?php echo $suppliers['suppliers_id'];?>)" style="cursor:pointer;"/>
                        </td>
                        <td align="center">
                            <a href="<?php echo U('index');?>&act=edit&id=<?php echo $suppliers['suppliers_id'];?>" title="编辑">编辑</a> |
                            <a href="javascript:void(0);" onclick="listTable.remove(<?php echo $suppliers['suppliers_id'];?>, '您确认要删除这条记录吗?')" title="移除">移除</a>      </td>
                    </tr>
