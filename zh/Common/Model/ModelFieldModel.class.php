...
class ModelFieldModel extends Model {
	...
	public function updateTableFieldCache() {
		...
		foreach ($dirArr as $dir) {
			if (is_dir($dir)) {
				if (!is_writable($dir)) {
					$this -> error = 'Fieldキャッシュディレクトリ修正権限ない';
					return false;
				} else {
					if (Dir::del($dir)) {
						$this -> error = 'Fieldキャッシュディレクトリ削除失敗';
						return false;
					}
				}
			}
