...
class ModelModel extends Model {
	...
	public function addModel($InsertData) {
		$this -> validate = array( 
            array('model_name', 'nonull', 'Model名は必須', 2, 1), 
            array('table_name', 'nonull', 'テーブル名は必須', 2, 1));
	...
	public function editModel($data) {
		$this -> validate = array( array('model_name', 'nonull', 'Model名は必須', 2, 2));
		if ($this -> create($data)) {
			if (!$this -> save($data)) {
				$this -> error = 'Model変更失敗';
			} else {
				if (!$this -> updateCache()) {
					return false;
				} else {
	...
	public function delModel($mid) {
		...
		if ( M('category') -> find($mid)) {
			$this -> error = '該当Model使われたカテゴリをまず削除してください';
		}
		$model = $this -> find($mid);
		if (is_null($model)) {
