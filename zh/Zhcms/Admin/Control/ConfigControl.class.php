...
class ConfigControl extends AuthControl{
	...
	function edit() {
       ...
       if (IS_POST) {
			...
			foreach ($config as $name => $c) {
                  ...
                  switch ($c['show_type']) {
					...
					case '布尔(1/0)' :
						...
						$config[$name]['html'] = "<label><input type='radio' name='{$c['name']}' value='1'  $Yes/> YES</label>
                                        <label><input type='radio' name='{$c['name']}' value='0' $No/> NO</label>";
						break;
					//多行文本
					case '多行文本' :
                    ...
                    case 'image':
                        ...
                        $temp_html.="<button class='zh-cancel-small' onclick='file_upload({\"id\":\"{$c['name']}\",\"type\":\"image\",\"dir\":\"config\",\"num\":1,\"name\":\"{$c['name']}\",\"filetype\":\"jpg,png,gif,jpeg\",\"upload_img_max_width\":\"800\",\"upload_img_max_height\":\"800\"})' type='button'>".'画像アップ'."</button>&nbsp;&nbsp;";
                        $temp_html.="<button class='zh-cancel-small' onclick='remove_upload_one_img(this)' type='button'>".'取り消し'."</button>";
                        $config[$name]['html'] = $temp_html;
                        break;
				}
            ...
            $ec_shop_country=C('ec_shop_country');
            if($ec_shop_country>0)
            {
                $this->assign('provinces', get_regions(1, $ec_shop_country));
                $ec_shop_province=C('ec_shop_province');
                if ($ec_shop_province)
                {
                    $this->assign('cities', get_regions(2, $ec_shop_province));
                }
            }
            $_CFG=C();
