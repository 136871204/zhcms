<?php
/**
 * ブランド管理
 * @author 周鸿 <136871204@qq.com>
 */
class EcAuctionControl extends EcAdminControl {
    //protected $db;
    
    public function __init() {
		//$this -> db = K("EcBrand");
	}
    
	//ブランド一覧
	public function index() {
	    $ecs=$GLOBALS['ec_globals']['ecs'];
	    //$image = new EcImage(C('ec_bgcolor'));
	    $tdb=M();
        $exc = new exchange($ecs->table('ec_goods_activity'), $tdb, 'act_id', 'act_name');
        
        
        $_LANG['auction_status'][PRE_START] = '未开始';
        $_LANG['auction_status'][UNDER_WAY] = '进行中';
        $_LANG['auction_status'][FINISHED] = '已结束';
        $_LANG['auction_status'][SETTLED] = '已结束';
        
        if ($_REQUEST['act'] == 'list'){
            /* 模板赋值 */
            $this->assign('full_page',   1);
            $this->assign('ur_here',     '拍卖活动列表');
            $this->assign('action_link', array('href' => U('index',array('act'=>'add')), 'text' => '添加拍卖活动'));
        
            $list = $this->auction_list();
        
            $this->assign('auction_list', $list['item']);
            $this->assign('filter',       $list['filter']);
            $this->assign('record_count', $list['record_count']);
            $this->assign('page_count',   $list['page_count']);
        
            $sort_flag  = sort_flag($list['filter']);
            $this->assign($sort_flag['tag'], $sort_flag['img']);
        
            /* 显示商品列表页面 */
            //assign_query_info();
            $this->display('auction_list.htm');
        }
        
        /*------------------------------------------------------ */
        //-- 分页、排序、查询
        /*------------------------------------------------------ */
        
        elseif ($_REQUEST['act'] == 'query')
        {
            $list = $this->auction_list();
        
            $this->assign('auction_list', $list['item']);
            $this->assign('filter',       $list['filter']);
            $this->assign('record_count', $list['record_count']);
            $this->assign('page_count',   $list['page_count']);
        
            $sort_flag  = sort_flag($list['filter']);
            $this->assign($sort_flag['tag'], $sort_flag['img']);
        
            make_json_result($this->fetch('auction_list.htm'), '',
                array('filter' => $list['filter'], 'page_count' => $list['page_count']));
        }
        
        /*------------------------------------------------------ */
        //-- 删除
        /*------------------------------------------------------ */
        elseif ($_REQUEST['act'] == 'remove')
        {
            $id = intval($_GET['id']);
            $auction = auction_info($id);
            if (empty($auction))
            {
                make_json_error('您要操作的拍卖活动不存在');
            }
            if ($auction['bid_user_count'] > 0)
            {
                make_json_error('该拍卖活动已经有人出价，不能删除');
            }
            $name = $auction['act_name'];
            $exc->drop($id);
    
            /* 记日志 */
            admin_log($name, '移除', '拍卖活动');
            
            /* 清除缓存 */
            clear_cache_files();
        
            $url = 'index.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
        
            ec_header("Location: $url\n");
            exit;
        }
        
        /*------------------------------------------------------ */
        //-- 批量操作
        /*------------------------------------------------------ */
        elseif ($_REQUEST['act'] == 'batch')
        {
            /* 取得要操作的记录编号 */
            if (empty($_POST['checkboxes']))
            {
                $this->sys_msg('没有选择记录');
            }
            else
            {
                $ids = $_POST['checkboxes'];
                if (isset($_POST['drop']))
                {
                    /* 查询哪些拍卖活动已经有人出价 */
                    $sql = "SELECT DISTINCT act_id FROM " . $ecs->table('ec_auction_log') .
                            " WHERE act_id " . db_create_in($ids);
                           // p($sql);
                    $tarray=M()->getCol($sql,'act_id');
                    if(!empty($tarray))
                    {
                        $ids = array_diff($ids, M()->getCol($sql,'act_id'));
                    }
                    
                    //p($ids);die;
                    if (!empty($ids))
                    {
                        /* 删除记录 */
                        $sql = "DELETE FROM " . $ecs->table('ec_goods_activity') .
                                " WHERE act_id " . db_create_in($ids) .
                                " AND act_type = '" . GAT_AUCTION . "'";
                        M()->exe($sql);
                        
                        /* 记日志 */
                        admin_log('', '批量删除', '拍卖活动');
                        /* 清除缓存 */
                        clear_cache_files();
                
                    }
                    $links[] = array('text' => '返回拍卖活动列表', 'href' => U('index').'&act=list&' . list_link_postfix());
                    $this->sys_msg('操作完成（已经有人出价的拍卖活动不能删除）', 0, $links);
                }
            }
        }

        /*------------------------------------------------------ */
        //-- 查看出价记录
        /*------------------------------------------------------ */
        
        elseif ($_REQUEST['act'] == 'view_log')
        {
            /* 参数 */
            if (empty($_GET['id']))
            {
                $this->sys_msg('参数错误');
            }
            $id = intval($_GET['id']);
            $auction = auction_info($id);
            if (empty($auction))
            {
                $this->sys_msg('您要操作的拍卖活动不存在');
            }
            $this->assign('auction', auction_info($id));
            /* 出价记录 */
            $this->assign('auction_log', auction_log($id));
            
            /* 模板赋值 */
            $this->assign('ur_here', '拍卖活动出价记录');
            $this->assign('action_link', array('href' => U('index').'&act=list&' . list_link_postfix(), 'text' => '拍卖活动列表'));
            //assign_query_info();
            $this->display('auction_log.htm');
        }
        
        /*------------------------------------------------------ */
        //-- 添加、编辑
        /*------------------------------------------------------ */
        
        elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
        {
            /* 是否添加 */
            $is_add = $_REQUEST['act'] == 'add';
            $this->assign('form_action', $is_add ? 'insert' : 'update');
            
            /* 初始化、取得拍卖活动信息 */
            if ($is_add)
            {
                $auction = array(
                    'act_id'        => 0,
                    'act_name'      => '',
                    'act_desc'      => '',
                    'goods_id'      => 0,
                    'product_id'    => 0,
                    'goods_name'    => '请先搜索商品',
                    'start_time'    => date('Y-m-d', time() + 86400),
                    'end_time'      => date('Y-m-d', time() + 4 * 86400),
                    'deposit'       => 0,
                    'start_price'   => 0,
                    'end_price'     => 0,
                    'amplitude'     => 0
                );
            }
            else
            {
                if (empty($_GET['id']))
                {
                    $this->sys_msg('参数错误');
                }
                $id = intval($_GET['id']);
                $auction = auction_info($id, true);
                if (empty($auction))
                {
                    $this->sys_msg('您要操作的拍卖活动不存在');
                }
                

                $auction['status'] = $_LANG['auction_status'][$auction['status_no']];
                $this->assign('bid_user_count', sprintf('已有 %s 个买家出价', $auction['bid_user_count']));
            }
            $this->assign('auction', $auction);
            /* 赋值时间控件的语言 */
            $this->assign('cfg_lang',$_SESSION['language']);
            /* 商品货品表 */
            $this->assign('good_products_select', get_good_products_select($auction['goods_id']));
            /* 显示模板 */
            if ($is_add)
            {
                $this->assign('ur_here', '添加拍卖活动');
            }
            else
            {
                $this->assign('ur_here', '编辑拍卖活动');
            }
            $this->assign('action_link', $this->list_link($is_add));
            //assign_query_info();
            $this->display('auction_info.htm');
    
        }
        /*------------------------------------------------------ */
        //-- 添加、编辑后提交
        /*------------------------------------------------------ */
        
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
        {
            /* 是否添加 */
            $is_add = $_REQUEST['act'] == 'insert';
            
            /* 检查是否选择了商品 */
            $goods_id = intval($_POST['goods_id']);
            if ($goods_id <= 0)
            {
                $this->sys_msg('请选择拍卖商品');
            }
            $sql = "SELECT goods_name FROM " . $ecs->table('ec_goods') . " WHERE goods_id = '$goods_id'";
            $row = M()->getRowSql($sql);
            if (empty($row))
            {
                $this->sys_msg('您要拍卖的商品不存在');
            }
            $goods_name = $row['goods_name'];
            
            /* 提交值 */
            $auction = array(
                'act_id'        => intval($_POST['id']),
                'act_name'      => empty($_POST['act_name']) ? $goods_name : sub_str($_POST['act_name'], 255, false),
                'act_desc'      => $_POST['act_desc'],
                'act_type'      => GAT_AUCTION,
                'goods_id'      => $goods_id,
                'product_id'    => empty($_POST['product_id']) ? 0 : $_POST['product_id'],
                'goods_name'    => $goods_name,
                'start_time'    => local_strtotime($_POST['start_time']),
                'end_time'      => local_strtotime($_POST['end_time']),
                'ext_info'      => serialize(array(
                            'deposit'       => round(floatval($_POST['deposit']), 2),
                            'start_price'   => round(floatval($_POST['start_price']), 2),
                            'end_price'     => empty($_POST['no_top']) ? round(floatval($_POST['end_price']), 2) : 0,
                            'amplitude'     => round(floatval($_POST['amplitude']), 2),
                            'no_top'     => !empty($_POST['no_top']) ? intval($_POST['no_top']) : 0
                        ))
            );
            /* 保存数据 */
            if ($is_add)
            {
                $auction['is_finished'] = 0;
                $insert_id=M()->autoExecute($ecs->table('ec_goods_activity'), $auction, 'INSERT');
                $auction['act_id'] =$insert_id;
            }
            else
            {
                M()->autoExecute($ecs->table('ec_goods_activity'), $auction, 'UPDATE', "act_id = '$auction[act_id]'");
            }
            /* 记日志 */
            if ($is_add)
            {
                admin_log($auction['act_name'], '添加', '拍卖活动');
            }
            else
            {
                admin_log($auction['act_name'], '编辑', '拍卖活动');
            }
            
            /* 清除缓存 */
            clear_cache_files();
            /* 提示信息 */
            if ($is_add)
            {
                $links = array(
                    array('href' => U('index').'&act=add', 'text' => '继续添加拍卖活动'),
                    array('href' => U('index').'&act=list', 'text' => '返回拍卖活动列表')
                );
                $this->sys_msg('添加拍卖活动成功', 0, $links);
            }
            else
            {
                $links = array(
                    array('href' => U('index').'&act=list&' . list_link_postfix(), 'text' => '返回拍卖活动列表')
                );
                $this->sys_msg('编辑拍卖活动成功', 0, $links);
            }
        }
        
        /*------------------------------------------------------ */
        //-- 处理冻结资金
        /*------------------------------------------------------ */
        
        elseif ($_REQUEST['act'] == 'settle_money')
        {
            /* 检查参数 */
            if (empty($_POST['id']))
            {
                $this->sys_msg('参数错误');
            }
            $id = intval($_POST['id']);
            $auction = auction_info($id);
            if (empty($auction))
            {
                $this->sys_msg('您要操作的拍卖活动不存在');
            }
            if ($auction['status_no'] != FINISHED)
            {
                $this->sys_msg('当前状态不正确');
            }
            if ($auction['deposit'] <= 0)
            {
                $this->sys_msg('没有保证金需要处理');
            }
            /* 处理保证金 */
            $exc->edit("is_finished = 2", $id); // 修改状态
            if (isset($_POST['unfreeze']))
            {
                /* 解冻 */
                log_account_change($auction['last_bid']['bid_user'], $auction['deposit'],
                    (-1) * $auction['deposit'], 0, 0, sprintf('解冻拍卖活动的保证金：%s', $auction['act_name']));
            }
            else
            {
                /* 扣除 */
                log_account_change($auction['last_bid']['bid_user'], 0,
                    (-1) * $auction['deposit'], 0, 0, sprintf('扣除拍卖活动的保证金：%s', $auction['act_name']));
            }
            /* 记日志 */
            admin_log($auction['act_name'], '编辑', '拍卖活动');
        
            /* 清除缓存 */
            clear_cache_files();
        
            /* 提示信息 */
            $this->sys_msg('处理冻结的保证金成功');
        }
        
        /*------------------------------------------------------ */
        //-- 搜索商品
        /*------------------------------------------------------ */
        
        elseif ($_REQUEST['act'] == 'search_goods')
        {

            $json   = new JSON;
            $filter = $json->decode($_GET['JSON']);
            $arr['goods']    = get_goods_list($filter);
        
            if (!empty($arr['goods'][0]['goods_id']))
            {
                $arr['products'] = get_good_products($arr['goods'][0]['goods_id']);
            }
        
            make_json_result($arr);
        }
        
        /*------------------------------------------------------ */
        //-- 搜索货品
        /*------------------------------------------------------ */
        
        elseif ($_REQUEST['act'] == 'search_products')
        {
            $json = new JSON;
        
            $filters = $json->decode($_GET['JSON']);
        
            if (!empty($filters->goods_id))
            {
                $arr['products'] = get_good_products($filters->goods_id);
            }
        
            make_json_result($arr);
        }

                
	}
    
    
    /*
     * 取得拍卖活动列表
     * @return   array
     */
    public function auction_list()
    {
        $result = get_filter();
        if ($result === false)
        {
            /* 过滤条件 */
            $filter['keyword']    = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
            if (isset($_REQUEST['is_ajax']) && $_REQUEST['is_ajax'] == 1)
            {
                $filter['keyword'] = json_str_iconv($filter['keyword']);
            }
            $filter['is_going']   = empty($_REQUEST['is_going']) ? 0 : 1;
            $filter['sort_by']    = empty($_REQUEST['sort_by']) ? 'act_id' : trim($_REQUEST['sort_by']);
            $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
            
            $where = "";
            if (!empty($filter['keyword']))
            {
                $where .= " AND goods_name LIKE '%" . mysql_like_quote($filter['keyword']) . "%'";
            }
            if ($filter['is_going'])
            {
                $now = gmtime();
                $where .= " AND is_finished = 0 AND start_time <= '$now' AND end_time >= '$now' ";
            }
    
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_goods_activity') .
                    " WHERE act_type = '" . GAT_AUCTION . "' $where";
            $filter['record_count'] = M()->getOne($sql,'COUNT(*)');
            
            /* 分页大小 */
            $filter = page_and_size($filter);
    
            /* 查询 */
            $sql = "SELECT * ".
                    "FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_goods_activity') .
                    " WHERE act_type = '" . GAT_AUCTION . "' $where ".
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT ". $filter['start'] .", $filter[page_size]";
    
            $filter['keyword'] = stripslashes($filter['keyword']);
            set_filter($filter, $sql);
            
        }
        else
        {
            $sql    = $result['sql'];
            $filter = $result['filter'];
        }
        $res = M()->query($sql);

        $list = array();
        if(!empty($res))
        {
            foreach($res as $row)
            {
                $ext_info = unserialize($row['ext_info']);
                $arr = array_merge($row, $ext_info);
        
                $arr['start_time']  = local_date('Y-m-d H:i', $arr['start_time']);
                $arr['end_time']    = local_date('Y-m-d H:i', $arr['end_time']);
        
                $list[] = $arr;
            }
        }
        $arr = array('item' => $list, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);

        return $arr;
    }
    
    
    
    /**
     * 列表链接
     * @param   bool    $is_add     是否添加（插入）
     * @param   string  $text       文字
     * @return  array('href' => $href, 'text' => $text)
     */
    function list_link($is_add = true, $text = '')
    {
        $href = U('index').'&act=list';
        if (!$is_add)
        {
            $href .= '&' . list_link_postfix();
        }
        if ($text == '')
        {
            $text = '拍卖活动列表';
        }
    
        return array('href' => $href, 'text' => $text);
    }
        
    
    
}
