...
class IndexControl extends PublicControl {
    ...
    {
        $company_login_id = Q('company_login_id');
        $company_pw = Q('post.company_pw', null, '');
        
        $company= M()->table('erp_company') -> where("company_login_id='$company_login_id' OR company_pw = 'company_pw'") -> find();
        ...
        $_SESSION = array_merge($_SESSION, $company);
        go(U('Erp/Company/index'));
    }
    
    //----------------------------------------
    public function setIndexShowData(){
        $db_prefix=C("DB_PREFIX");
        $db =ContentViewModel::getInstance(12);
        $table=$db->tableFull;
        $where=array();
        $join = "content_flag,category,user";
        $result= $db->join($join)->order("arc_sort ASC")->all();
