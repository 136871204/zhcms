...
class ViewTag {
     ...
     */
     public $tag = array(
        ...
        'html_radios' => array("block" => 0),
        'html_select_date' => array("block" => 0),
     );
...
//        $ueditor_path = __ZHPHP_EXTEND__ . '/Org/Editor/Ueditor/'; //url路径
    ...
    //zhjs
    public function _zhecjs($attr, $content)
    {
        //导入jquery框架，zhjs.css ,zhjs.js
        $php = '';
        //$php .= "<script type='text/javascript' src='__ZHPHP_EXTEND__/Org/Jquery/jquery-1.8.2.min.js'></script>\n";
    ...
    function _html_radios($attr,$content){
        $options = $attr['options'];
        $checked = isset($attr['checked']) ? $attr['checked'] : 'null'; //无数据时
        $name = $attr['name'];
        $php = '<?php ';
        $php .= 'if(isset(' . $options . ') && !empty(' . $options . ')):';
        $php .= '$arr["options"]=' . $options . ';';
        $php .= '$arr["checked"]='.$checked.';';
        $php .= '$arr["name"]='.$name.';';
        $php .= 'echo html_radios($arr);';
        $php .= 'endif;' . "\n";
        $php .= '?>';
        return $php;
    }
    
    function _html_select_date($attr,$content){
        /*
        {html_select_date 
        prefix="end_date" 
        field_order="YMD" 
        ...
        */
        ...
        $month_format = $attr['month_format'];
        $day_value_format = $attr['day_value_format'];
        $start_year = $attr['start_year'];
        $end_year = $attr['end_year'];
        $display_days = $attr['display_days'];
        $time = $attr['time'];
        
        $php = '<?php ';
        $php .= '$arr["prefix"]="' . $prefix . '";';
        $php .= '$arr["field_order"]="'.$field_order.'";';
        $php .= '$arr["month_format"]="'.$month_format.'";';
        $php .= '$arr["day_value_format"]="' . $day_value_format . '";';
        $php .= '$arr["start_year"]="'.$start_year.'";';
        $php .= '$arr["end_year"]="'.$end_year.'";';
        $php .= '$arr["display_days"]="' . $display_days . '";';
        $php .= '$arr["time"]='.$time.';';
        $php .= 'echo html_select_date($arr);';
        $php .= '?>';
        return $php;
    }
