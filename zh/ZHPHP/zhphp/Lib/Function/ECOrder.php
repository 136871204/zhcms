...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    ...
    {
        case 0 :
            $sql = "SELECT goods_id, SUM(send_number) AS num, MAX(extension_code) AS extension_code, product_id 
                    FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_order_goods') .
                    " WHERE order_id = '$order_id' AND is_real = 1 GROUP BY goods_id, product_id";
        break;

        case 1 :
            $sql = "SELECT goods_id, SUM(goods_number) AS num, MAX(extension_code) AS extension_code, product_id 
                    FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_order_goods') .
                    " WHERE order_id = '$order_id' AND is_real = 1 GROUP BY goods_id, product_id";
        break;
    }
        ...
        foreach($res as $row)
        {
            ...
            else
            {
                $sql = "SELECT goods_id, goods_number" .
                   " FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_package_goods') .
...
function get_agency_by_regions($regions)
{
    ...
    $sql = "SELECT region_id, agency_id " .
            "FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_region') .
...
{
    $sql = "SELECT SUM(goods_price * goods_number) as re " .
    ...
    return floatval(M()->getOne($sql,'re'));
}
...
function user_bonus($user_id, $goods_amount = 0)
{
    ...
    $sql = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id " .
            "FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_bonus_type') . " AS t," .
                $GLOBALS['ec_globals']['ecs']->table('ec_user_bonus') . " AS b " .
...
function flow_clear_cart_alone()
{
    ...
    $sql = "SELECT c.rec_id, gg.parent_id
            FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_cart') . " AS c
                    LEFT JOIN " . $GLOBALS['ec_globals']['ecs']->table('ec_group_goods') . " AS gg ON c.goods_id = gg.goods_id
                    LEFT JOIN" . $GLOBALS['ec_globals']['ecs']->table('ec_goods') . " AS g ON c.goods_id = g.goods_id
...
function card_list()
{
    $sql = "SELECT * FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_card');
    $res = M()->query($sql);
...
function pack_list()
{
    $sql = 'SELECT * FROM ' . $GLOBALS['ec_globals']['ecs']->table('ec_pack');
    $res = M()->query($sql);
...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false)
{
    $sql = 'SELECT pay_id, pay_code, pay_name, pay_fee, pay_desc, pay_config, is_cod' .
            ' FROM ' . $GLOBALS['ec_globals']['ecs']->table('ec_payment') .
            ' WHERE enabled = 1 ';
    if (!$support_cod)
    {
...
function cart_weight_price($type = CART_GENERAL_GOODS)
{
    ...
    /* 计算超值礼包内商品的相关配送参数 */
    $sql = 'SELECT goods_id, goods_number, goods_price 
            FROM ' . $GLOBALS['ec_globals']['ecs']->table('ec_cart') . " 
            WHERE extension_code = 'package_buy' AND session_id = '" . SESS_ID . "'";
    $row = M()->getAll($sql);
    
    if ($row)
    {
        echo "TODO:ECOrder----cart_weight_price--1";die;   
    }
    /* 获得购物车中非超值礼包商品的总重量 */
...
function add_package_to_cart($package_id, $num = 1)
{
    ...
    if (empty($package))
    {
        $GLOBALS['ec_globals']['err']->add('对不起，指定的商品不存在', ERR_NOT_EXISTS);
    ...
    {
        $GLOBALS['ec_globals']['err']->add('对不起，该商品已经下架。', ERR_NOT_ON_SALE);

        return false;
    }
    /* 现有库存是否还能凑齐一个礼包 */
    if (C('ec_use_storage') == '1' && judge_package_stock($package_id))
    {
        $GLOBALS['ec_globals']['err']->add(sprintf("对不起，该商品已经库存不足暂停销售。\n你现在要进行缺货登记来预订该商品吗？", 1), ERR_OUT_OF_STOCK);
    ...
    {
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT goods_number FROM " .$GLOBALS['ec_globals']['ecs']->table('ec_cart').
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            if (C('ec_use_storage') == 0 || $num > 0)
            {
                $sql = "UPDATE " . $GLOBALS['ec_globals']['ecs']->table('ec_cart') . " SET goods_number = '" . $num . "'" .
                       " WHERE session_id = '" .SESS_ID. "' AND goods_id = '$package_id' ".
                       " AND parent_id = 0 AND extension_code = 'package_buy' " .
                       " AND rec_type = '" . CART_GENERAL_GOODS . "'";
                M()->exe($sql);
        ...
        {
            M()->autoExecute($GLOBALS['ec_globals']['ecs']->table('ec_cart'), $parent, 'INSERT');
        }
    }
    
    /* 把赠品删除 */
    $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_cart') . " WHERE session_id = '" . SESS_ID . "' AND is_gift <> 0";
    M()->exe($sql);
...
function available_shipping_list($region_id_list)
{
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            'FROM ' . $GLOBALS['ec_globals']['ecs']->table('ec_shipping') . ' AS s, ' .
                $GLOBALS['ec_globals']['ecs']->table('ec_shipping_area') . ' AS a, ' .
                $GLOBALS['ec_globals']['ecs']->table('ec_area_region') . ' AS r ' .
            'WHERE r.region_id ' . db_create_in($region_id_list) .
            ' AND 
                r.shipping_area_id = a.shipping_area_id AND 
                a.shipping_id = s.shipping_id AND 
                s.enabled = 1 ORDER BY s.shipping_order';
...
function order_fee($order, $goods, $consignee)
{
    ...
    {
        echo 'TODO:ECOrder---order_fee1';die;
    ...
    if (!empty($order['need_inv']) && $order['inv_type'] != '')
    {
        ...
        foreach ($invoice_type_type as $key => $type)
        {
            ...
            {
                $rate = floatval($invoice_type_rate[$key]) / 100;
                break;
            }
        }
        if ($rate > 0)
    ...
    {
        //echo 'TODO:ECOrder---order_fee4';die;
    ...
    {
        echo 'TODO:ECOrder---order_fee6';die;
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            if ($order['extension_code'] == 'group_buy')
            {
                echo 'TODO:ECOrder---order_feea';die;
            }
            else
            {
            ...
            $total['shipping_fee'] = ($shipping_count == 0 AND $weight_price['free_shipping'] == 1) ?0 :  
    ...
    {
        echo 'TODO:ECOrder---order_fee8';die;
    ...
    $se_flow_type = isset($_SESSION['ec_flow_type']) ? $_SESSION['ec_flow_type'] : '';
    ...
    if ($order['extension_code'] == 'group_buy')
    {
        echo 'TODO:ECOrder---order_fee12';die;
    ...
    {
        $sql = 'SELECT SUM(eg.exchange_integral) as t_sum '.
               'FROM ' . $GLOBALS['ec_globals']['ecs']->table('ec_cart') . ' AS c,' . 
                        $GLOBALS['ec_globals']['ecs']->table('ec_exchange_goods') . 'AS eg '.
        ...
        $exchange_integral = M()->getOne($sql,'t_sum');
...
function value_of_integral($integral)
{
    ...
    $scale = floatval($ec_integral_scale);
...
{
        $sql = "SELECT SUM(c.goods_number * IF(g.give_integral > -1, g.give_integral, c.goods_price)) as re " .
                "FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_cart') . " AS c, " .
                          $GLOBALS['ec_globals']['ecs']->table('ec_goods') . " AS g " .
                "WHERE c.goods_id = g.goods_id " .
                "AND c.session_id = '" . SESS_ID . "' " .
...
function get_total_bonus()
{
    ...
    $today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
    /* 按商品发的红包 */
    $sql = "SELECT SUM(c.goods_number * t.type_money) as re " .
            "FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_cart') . " AS c, "
                    . $GLOBALS['ec_globals']['ecs']->table('ec_bonus_type') . " AS t, "
                    . $GLOBALS['ec_globals']['ecs']->table('ec_goods') . " AS g " .
            "WHERE c.session_id = '" . SESS_ID . "' " .
            "AND c.is_gift = 0 " .
    ...
    $goods_total = floatval(M()->getOne($sql,'re'));
    ...
    $sql = "SELECT SUM(goods_price * goods_number) as re  " .
            ...
            " AND is_gift = 0 " .
            " AND rec_type = '" . CART_GENERAL_GOODS . "'";
    $amount = floatval(M()->getOne($sql,'re'));
    
    /* 按订单发的红包 */
    $sql = "SELECT FLOOR('$amount' / min_amount) * type_money as re  " .
            "FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_bonus_type') .
            " WHERE send_type = '" . SEND_BY_ORDER . "' " .
            " AND send_start_date <= '$today' " .
            "AND send_end_date >= '$today' " .
            "AND min_amount > 0 ";
    $order_total = floatval(M()->getOne($sql,'re'));
...
{
    $scale = floatval(C('ec_integral_scale'));
...
{
    $order = isset($_SESSION['ec_flow_order']) ? $_SESSION['ec_flow_order'] : array();
    //p($order);die;
    /* 初始化配送和支付方式 */
...
{
    if (isset($_SESSION['ec_flow_consignee']))
    {
        /* 如果存在session，则直接返回session中的收货人信息 */

        return $_SESSION['ec_flow_consignee'];
    }
    else
    {
        /* 如果不存在，则取得用户的默认收货人信息 */
        ...
        if ($user_id > 0)
        {
            /* 取默认地址 */
            $sql = "SELECT ua.*".
                    " FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_user_address') . "AS ua, ".
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    if (empty($goods))
    {
        $GLOBALS['err']->add('对不起，指定的商品不存在', ERR_NOT_EXISTS);
        return false;
    }
    /* 如果是作为配件添加到购物车的，需要先检查购物车里面是否已经有基本件 */
    if ($parent > 0)
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_cart') .
                " WHERE goods_id='$parent' AND session_id='" . SESS_ID . "' AND extension_code <> 'package_buy'";  
        if (M()->getOne($sql,'COUNT(*)') == 0)
        {
            $GLOBALS['ec_globals']['err']->add('对不起，您希望将该商品做为配件购买，可是购物车中还没有该商品的基本件。', ERR_NO_BASIC_GOODS);
    ...
    {
        $GLOBALS['ec_globals']['err']->add('对不起，不在销售', ERR_NOT_ON_SALE);
        return false;
    }
    /* 不是配件时检查是否允许单独销售 */
    if (empty($parent) && $goods['is_alone_sale'] == 0)
    {
        $GLOBALS['ec_globals']['err']->add('对不起，不能单独销售', ERR_CANNT_ALONE_SALE);
        return false;
    }
    /* 如果商品有规格则取规格商品信息 配件除外 */
    $sql = "SELECT * FROM " .$GLOBALS['ec_globals']['ecs']->table('ec_products'). " WHERE goods_id = '$goods_id' LIMIT 0, 1";
    $prod = M()->getRowSql($sql);
    ...
    /* 检查：库存 */
    if (C('ec_use_storage')== 1)
    {
        //检查：商品购买数量是否大于总库存
        if ($num > $goods['goods_number'])
        {
            $GLOBALS['ec_globals']['err']->add(sprintf("对不起，该商品已经库存不足暂停销售。\n你现在要进行缺货登记来预订该商品吗？", $goods['goods_number']), ERR_OUT_OF_STOCK);
        ...
        if (is_spec($spec) && !empty($prod))
        {
            if (!empty($spec))
            {
                ...
                {
                    $GLOBALS['ec_globals']['err']->add(sprintf("对不起，该商品已经库存不足暂停销售。\n你现在要进行缺货登记来预订该商品吗？", $product_info['product_number']), ERR_OUT_OF_STOCK);
                    return false;
                }
            }
        }
    ...
    $sql = "SELECT parent_id, goods_price " .
            "FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_group_goods')  .
    ...
    if ($basic_list)
    {
        echo __FILE__.'---'.__METHOD__.'$basic_list';die;
    }
    /* 取得购物车中该商品每个基本件已有该商品配件数量，计算出每个基本件还能有几个该商品配件 */
    /* 一个基本件对应一个该商品配件 */
    if ($basic_count_list)
    {
        echo __FILE__.'---'.__METHOD__.'$basic_count_list';die;
    }
    /* 循环插入配件 如果是配件则用其添加数量依次为购物车中所有属于其的基本件添加足够数量的该配件 */
    foreach ($basic_list as $parent_id => $fitting_price)
    {
        echo __FILE__.'---'.__METHOD__.'循环插入配件';die;
    }
    /* 如果数量不为0，作为基本件插入 */
    if ($num > 0)
    {
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT 
                    goods_number FROM " .$GLOBALS['ec_globals']['ecs']->table('ec_cart').
        ...
        if($row)
        {
            ...
            if (C('ec_use_storage')== 0 || $num <= $goods_storage)
            {
                
                $goods_price = get_final_price($goods_id, $num, true, $spec);
                $sql = "UPDATE " . 
                            $GLOBALS['ec_globals']['ecs']->table('ec_cart')  . " SET goods_number = '$num'" .
            ...
            {
                $GLOBALS['ec_globals']['err']->add(sprintf("すみません、商品在庫が足りない販売停止する。\n今は在庫足りないを登録して商品予定するか？", $num), ERR_OUT_OF_STOCK);
                return false;
            }
...
{
    $sql = "SELECT SUM(goods_price * goods_number) as result " .
            "FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_order_goods') .
    ...
    return floatval(M()->getOne($sql,'result'));
}
...
function exist_real_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS)
{
    ...
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_order_goods') .
...
function order_weight_price($order_id)
{
    $sql = "SELECT SUM(g.goods_weight * o.goods_number) AS weight, " .
            ...
            "FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_order_goods') . " AS o, " .
                $GLOBALS['ec_globals']['ecs']->table('ec_goods') . " AS g " .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_users') .
...
function order_query_sql($type = 'finished', $alias = '')
{
    ...
    elseif ($type == 'await_ship')
    {
        return " AND   {$alias}order_status " .db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART)) .
               " AND   {$alias}shipping_status " .db_create_in(array(SS_UNSHIPPED, SS_PREPARING, SS_SHIPPED_ING)) .
...
function get_cart_goods()
{
    ...
    $sql = "SELECT *, IF(parent_id, parent_id, goods_id) AS pid " .
            " FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_cart')  . " " .
            " WHERE session_id = '" . session_id() . "' AND rec_type = '" . CART_GENERAL_GOODS . "'" .
    ...
    if(!empty($res)){
        foreach($res as $row){
            ...
            if (trim($row['goods_attr']) != '')
            {
                ...
                $sql = "SELECT attr_value FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_goods_attr') . " WHERE goods_attr_id " .
                db_create_in($row['goods_attr']);
                $attr_list = M()->getCol($sql,'attr_value');
                if(!empty($attr_list)){
            ...
            $show_goods_in_cart=C('ec_show_goods_in_cart');
            /* 增加是否在购物车里显示商品图 */
            if (($show_goods_in_cart == "2" || $show_goods_in_cart == "3") && $row['extension_code'] != 'package_buy')
            {
                $goods_thumb = M()->getOne("SELECT `goods_thumb` FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_goods')  . " WHERE `goods_id`='{$row['goods_id']}'",'goods_thumb');
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0)
{
    ...
    elseif (2 == $refund_type)
    {
        ...
        $account = array(
            ...
            'admin_note'   => sprintf('订单退款：%s', $order['order_sn']),
            'is_paid'      => 0
        );
        M()->autoExecute($GLOBALS['ec_globals']['ecs']->table('ec_user_account'), $account, 'INSERT');
...
{
    return M()->autoExecute($GLOBALS['ec_globals']['ecs']->table('ec_order_info'),
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['ec_globals']['ecs']->table('ec_payment') .
...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['ec_globals']['ecs']->table('ec_shipping') .
...
{
    /* 计算订单各种费用之和的语句 */
    //商品总金额-折扣金额+发票税额+配送费用+报价费用+（支付费用,跟支付方式的配置相关，取值表ec_payment）+包装费用+贺卡费用
    $total_fee = " (goods_amount - discount + tax + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee) AS total_fee ";
    $order_id = intval($order_id);
    if ($order_id > 0)
    {
        $sql = "SELECT *, " . $total_fee . " FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_order_info') .
                " WHERE order_id = '$order_id'";
    }
    else
    {
    ...
    if ($order)
    {
        ...
        $order['formated_add_time']       = local_date(C('ec_time_format'), $order['add_time']);
    }
...
function judge_package_stock($package_id, $package_num = 1)
{
    ...
    if ($goods['product_ids'] != '')
    {
        $sql = "SELECT p.product_id
                FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_products') . " AS p, " . 
