...
class integrate
{
    ...
    function integrate($cfg)
    {
        ...
        $this->db_name = isset($db_database) ? $db_database : '';
        $this->prefix = isset($db_prefix) ? $db_prefix : '';
        $this->db=M();
        $this->need_sync = true;
        ...
        {
            $this->db_name =C('DB_DATABASE');// $GLOBALS['ecs']->db_name;
            $this->prefix = C('DB_PREFIX');//$GLOBALS['ecs']->prefix;
            $this->db = M();
            //p($this);die;
        }
        else
...
/*
    ...
    function add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date=0, $md5password='')
    {
        ...
        if ($this->db->getOne($sql, $this->field_id) > 0)
        {
       ...
       //echo 'integrate----add_user1 处理session';die;
        $this->db->exe($sql);
        //$this->sync($username, $password);
        if ($this->need_sync)
        {
    ...
    function edit_user($cfg)
    {
        ...
        if ((!empty($cfg['email'])) && $this->field_email != 'NULL')
        {
            ...
            if ($this->db->getOne($sql,$this->field_id, true) > 0)
            {
                $this->error = ERR_EMAIL_EXISTS;
            ...
            $sql = "SELECT count(*)" .
                   " FROM " . $this->table($this->user_table).
                   " WHERE " . $this->field_email . " = '$cfg[email]' ";
            if($this->db->getOne($sql,'count(*)', true) == 0)
            {
                // 新的E-mail
                $sql = "UPDATE " . $GLOBALS['ec_globals']['ecs']->table('ec_users') . " SET is_validated = 0 WHERE user_name = '$cfg[post_username]'";
                $this->db->exe($sql);
            }
    ...
    function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_ecshop) && $this->is_ecshop))
        {
            ...
            $sql = "SELECT user_id FROM "  . $GLOBALS['ec_globals']['ecs']->table('ec_users') . " WHERE ";
            ...
            if ($col)
            {
                $sql = "UPDATE " . $GLOBALS['ec_globals']['ecs']->table('ec_users') . " SET parent_id = 0 WHERE " . db_create_in($col, 'parent_id'); //将删除用户的下级的parent_id 改为0
                M()->exe($sql);
                
                $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_users') . " WHERE " . db_create_in($col, 'user_id'); //删除用户
                M()->exe($sql);
                

                $sql = "SELECT order_id FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_order_info') . " WHERE " . db_create_in($col, 'user_id');
                M()->exe($sql);
                
                $col_order_id = M()->getColSql($sql);
                if ($col_order_id)
                {
                    $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_order_info') . " WHERE " . db_create_in($col_order_id, 'order_id');
                    M()->exe($sql);
                    $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_order_goods') . " WHERE " . db_create_in($col_order_id, 'order_id');
                    M()->exe($sql);
                }

                $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_booking_goods') . " WHERE " . db_create_in($col, 'user_id'); //删除用户
                M()->exe($sql);
                $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_collect_goods') . " WHERE " . db_create_in($col, 'user_id'); //删除会员收藏商品
                M()->exe($sql);
                $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_feedback') . " WHERE " . db_create_in($col, 'user_id'); //删除用户留言
                M()->exe($sql);
                $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_user_address') . " WHERE " . db_create_in($col, 'user_id'); //删除用户地址
                M()->exe($sql);
                $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_user_bonus') . " WHERE " . db_create_in($col, 'user_id'); //删除用户红包
                M()->exe($sql);
                $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_user_account') . " WHERE " . db_create_in($col, 'user_id'); //删除用户帐号金额
                M()->exe($sql);
                $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_tag') . " WHERE " . db_create_in($col, 'user_id'); //删除用户标记
                M()->exe($sql);
                $sql = "DELETE FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_account_log') . " WHERE " . db_create_in($col, 'user_id'); //删除用户日志
                M()->exe($sql);
            }
        }
...
//p($row);die;
    ...
    function set_cookie($username='', $remember= null )
    {
        if (empty($username))
        {
            /* 摧毁cookie */
            $time = time() - 3600;
            setcookie("ECS[ec_user_id]",  '', $time, $this->cookie_path);            
            setcookie("ECS[ec_password]", '', $time, $this->cookie_path);
        ...
        elseif ($remember)
        {
            ...
            setcookie("ECS[ec_username]", $username, $time, $this->cookie_path, $this->cookie_domain);
            $sql = "SELECT user_id, password FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_users') . " WHERE user_name='$username' LIMIT 1";
            $row = M()->getRowSql($sql);
            if ($row)
            {
                setcookie("ECS[ec_user_id]", $row['user_id'], $time, $this->cookie_path, $this->cookie_domain);
                setcookie("ECS[ec_password]", $row['password'], $time, $this->cookie_path, $this->cookie_domain);
            }
        }
    }
    ...
    function set_session ($username='')
    {
        if (empty($username))
        {
            //$GLOBALS['ec_globals']['sess']->destroy_session();
            unset($_SESSION['ec_user_id']);
            unset($_SESSION['ec_user_name']);
            unset($_SESSION['ec_email']);
        }
        else
        {
            $sql = "SELECT user_id, password, email FROM " . $GLOBALS['ec_globals']['ecs']->table('ec_users') . " WHERE user_name='$username' LIMIT 1";
            $row = M()->getRowSql($sql);

            if ($row)
            {
                $_SESSION['ec_user_id']   = $row['user_id'];
                $_SESSION['ec_user_name'] = $username;
    ...
    function sync ($username, $password='', $md5password='')
    {
        ...
        //p($sql);die;
        $profile = M()->getRowSql($sql);
        if (empty($profile))
        {
            /* 向商城表插入一条新记录 */
            if (empty($md5password))
            {
               $sql = "INSERT INTO " . $GLOBALS['ec_globals']['ecs']->table('ec_users').
